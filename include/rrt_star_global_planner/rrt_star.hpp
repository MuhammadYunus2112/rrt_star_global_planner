...
class RRTStar {
 public:
  ...
  std::list<std::pair<float, float>> getGlobalPath() const;

  std::pair<float,float> normalizeVector(std::pair<float,float> vec);

  std::pair<float,float> elipsoidSample(std::pair<float, float> start, std::pair<float, float> goal, float c_max);
  std::vector<float> getElips();
 ...
 private:
  ...
  RandomDoubleGenerator unit_random_double_;
  int node_count_{0};
  float map_width_;
  float map_height_;
  double radius_;
  ...
  std::list<std::pair<float, float>> global_path;
  float c_best = 99999999;
